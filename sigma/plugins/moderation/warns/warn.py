...
async def warn(cmd, message, args):
    ...
    if warned_users[target.id]['Warns'] > warn_limit:
        ...
        out_content_local.add_field(name=':boot: User **' + target.name + '** has been kicked!', value='Reasons:\n```\n' + '\n'.join(warned_users[target.id]['Reasons']) + '\n```')
        
        await cmd.bot.send_message(message.channel, None, embed=out_content_local)
        out_content_to_user = discord.Embed(color=0x993300)
        out_content_to_user.add_field(name=':boot: You have been kicked!', value='Reasons:\n```\n' + '\n'.join(warned_users[target.id]['Reasons']) + '\n```')
        ...
        out_content_to_user.add_field(name=':warning: Warning ' + str(warned_users[target.id]['Warns']) + '/' + str(warn_limit) + ' on ' + message.server.name, value='Reason:\n```\n' + warning_text + '\n```')
        
        await cmd.bot.send_message(target, None, embed=out_content_to_user)
        out_content_local = discord.Embed(color=0xFF9900, title=':warning: Warning ' + str(warned_users[target.id]['Warns']) + '/' + str(warn_limit) + ' for ' + target.name)
