...
async def greetpm(cmd, message, args):
    ...
    if active:
        ...
        out_content.add_field(name='✅ Success', value='Greeting via private message has been disabled.')
    else:
        cmd.db.set_settings(message.guild.id, 'GreetPM', True)
        out_content.add_field(name='✅ Success', value='Greeting via private message has been enabled.')
