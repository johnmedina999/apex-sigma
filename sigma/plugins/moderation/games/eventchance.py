...
async def eventchance(cmd, message, args):
    ...
    if not check_admin(message.author, message.channel):
        out_content = discord.Embed(type='rich', color=0xDB0000, title=':no_entry: Insufficient Permissions. Server Admin Only.')
    ...
    if not args:
        out_content = discord.Embed(color=0x0099FF, title=':information_source: Random Event Chance Is ' + str(event_chance) + '%')
    ...
    try: new_chance = int(new_chance)
    except:
        out_content = discord.Embed(type='rich', color=0xDB0000, title=':exclamation: Invalid Number Input.')
        await message.channel.send(None, embed=out_content)
        return
        
    if new_chance > 100: new_chance = 100
