...
async def prune(cmd, message, args):
    ...
    if not check_man_msg(message.author, message.channel):
        embed = discord.Embed(title='⚠ Unpermitted. Only Those With The Manage Message Permission Allowed.', color=0xDB0000)
    ...
    if target: deleted = await message.channel.purge(limit=limit, check=author_check)
    else: deleted = await message.channel.purge(limit=limit)
    ...
    if log_channel_id:
        ...
        if log_channel:
            ...
            if target: target_text = f'{target.mention}'
            else: target_text = 'No Filter'
