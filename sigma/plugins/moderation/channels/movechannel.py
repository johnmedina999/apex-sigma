...
async def movechannel(cmd, message, args):
    ...
    if not check_man_chan(message.author, message.channel):
        embed = discord.Embed(type='rich', color=0xDB0000, title='⛔ Insufficient Permissions. Requires Manage Channels Permission Only.')
    ...
    embed = discord.Embed(color=0x66CC66, title='✅ ' + message.channel.name + ' moved from ' + str( pos_pre) + ' to ' + str(position))
    await message.channel.send(None, embed=embed)