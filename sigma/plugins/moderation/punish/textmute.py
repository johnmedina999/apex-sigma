...
async def textmute(cmd, message, args):
    ...
    if log_channel_id:
        ...
        if log_channel:
            ...
            log_embed.add_field(name='🔇 Muted User', value=f'{target.mention}\n{target.name}#{target.discriminator}', inline=True)            
    ...
    response = discord.Embed(color=0x66CC66, title=f'✅ {target.name}#{target.discriminator} has been text muted.')
    await message.channel.send(embed=response)
