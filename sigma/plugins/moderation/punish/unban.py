...
async def unban(cmd, message, args):
    ...
    await message.guild.unban(target, reason=f'Unbanned by {message.author.name}#{message.author.discriminator}.')
    ...
    if log_channel_id:
        ...
        if log_channel:
            ...
            log_response.add_field(name='🔨 Unbanned User', value=f'{target.mention}\n{target.name}#{target.discriminator}', inline=True)            
            log_response.add_field(name='🛡 Responsible', value=f'{author.mention}\n{author.name}#{author.discriminator}', inline=True)
