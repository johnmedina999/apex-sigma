...
async def queue(cmd, message, args):
    ...
    if not message.author.voice:
        embed = discord.Embed(color=0x994444, title=':exclamation: Join a voice channel first!')
    ...
    if args:
        ...
        if '?list=' in qry:
            ...
            if sound_type == 0:
                ...
                embed.set_author(name=f'{message.author.name}#{message.author.discriminator}', icon_url=user_avatar(message.author))
