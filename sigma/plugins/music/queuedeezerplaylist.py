...
async def queuedeezerplaylist(cmd, message, args):
    ...
    detection_response = discord.Embed(color=0xFFCC66, title=f'💽 Importing {len(playlist_items)} songs from {playlist_name}...')
    ...
    finished_response = discord.Embed(color=0xFFCC66, title=f'💽 All Done! {len(playlist_items)} songs were imported!')
