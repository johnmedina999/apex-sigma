...
async def searchid(cmd, message, args):
    ...
    if not message.author.id in permitted_id:
        out = discord.Embed(type='rich', color=0xDB0000, title='⛔ Insufficient Permissions. Bot Owner Only.')
    ...
    if count == 0: embed = discord.Embed(title='ℹ User Not Found', color=0x0099FF)
