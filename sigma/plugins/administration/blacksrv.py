...
async def blacksrv(cmd, message, args):
    ...
    if not target:
        embed = discord.Embed(type='rich', color=0xDB0000, title='❗ No server by that ID was found.')
    ...
    if black:
        ...
        embed = discord.Embed(title=':unlock: Server ' + target.name + ' has been un-blacklisted.', color=0xFF9900)
    else:
