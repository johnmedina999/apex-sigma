...
async def togglerole(cmd, message, args):
    ...
    if not target_role:
        ...
        out_content.add_field(name='Role Not Found', value='I was unable to find that role on this server.')
        await message.channel.send(None, embed=out_content)
    ...
    if not self_role_id:
        out_content = discord.Embed(type='rich', color=0xFF9900, title='⚠ Error')
        out_content.add_field(name='Role Not Found', value='I was unable to find that role in the list of self assignable roles for this server.')
        await message.channel.send(None, embed=out_content)
        return
    ...
    if not user_role_match:
        ...
        embed = discord.Embed(title='✅ ' + target_role.name + ' has been added to you.', color=0x66cc66)
    else:
        await message.author.remove_roles(target_role)
        embed = discord.Embed(title='⚠ ' + target_role.name + ' has been removed from you.', color=0xFF9900)
