...
async def takerole(cmd, message, args):
    ...
    if not user_contained_role:
        out_content = discord.Embed(type='rich', color=0xFF9900, title='❗ Error')
        out_content.add_field(name='User Missing Role', value='I was unable to find **' + role_qry + '** in ' + target_user.name + '\'s roles.')
        await message.channel.send(None, embed=out_content)
        
    await target_user.remove_roles(target_role)
    out_content = discord.Embed(type='rich', color=0x66cc66, title='✅ Role ' + role_qry + ' removed from **' + target_user.name + '**.')
    await message.channel.send(None, embed=out_content)