...
async def keyvis(cmd, message, args):
    ...
    try: item = key_vn_list[choice]
    except KeyError:
        embed = discord.Embed(color=0x696969, title='🔍 Nothing found for {:s}...'.format(' '.join(['`{:s}`'.format(x) for x in args])))
    ...
    image_url = '{:s}/{:s}/{:s}{:d}.jpg'.format(url_base, item[0], '0000'[:-ran_number_length], ran_image_number)
