...
async def blackjack(cmd, message, args):
    ...
    if bj_instance:
        ...
        embed.set_footer(text=f'Use {Prefix}bjnext to draw the next card, {Prefix}bjfold to fold the current hand or {Prefix}bjquit to quit the game.')
        await message.channel.send(None, embed=embed)
        return
    ...
    embed = discord.Embed(color=0x1abc9c, title=random.choice(symbols) + ' A BlackJack Instance Has Been Created For You.')
