...
async def manga(cmd, message, args):
    ...
    try:
        ...
        try: synopsis = entries[ani_no][11].text.replace('[i]', '').replace('[/i]', '').replace('<br>', '').replace('</br>', '').replace('<br />', '').replace('&#039;', '\'').replace('&quot;', '"').replace('&mdash;', '-')
        ...
        try: overlay = Image.open(cmd.resource('img/overlay_manga.png')); 
        except: cmd.log.error("Could not open img/overlay_manga.png"); raise
