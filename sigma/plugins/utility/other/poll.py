...
async def poll(cmd, message, args):
    ...
    if len(poll_choices) < 2:
        out_content = discord.Embed(type='rich', color=0xDB0000, title='❗ Not enough arguments present.')
        await message.channel.send(None, embed=out_content)
        return
    
    if len(poll_choices) > 9:
        out_content = discord.Embed(type='rich', color=0xDB0000, title='❗ Maximum is 9 choices.')
        await message.channel.send(None, embed=out_content)
        return
