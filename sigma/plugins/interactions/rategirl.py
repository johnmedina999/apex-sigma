    ...
    with Image.open(cmd.resource('rate/crazy_hot_chart.png')) as chart:
        if target.bot:
            ...
            if lookup:
                ...
                if hot_value < 10: hot_value = f'0{hot_value}'
                else: hot_value = str(hot_value)
                
                if crazy_value < 10: crazy_value = f'0{crazy_value}'
                else: crazy_value = str(crazy_value)
        ...
        avatar_img = avatar_img.resize(ava_size, Image.ANTIALIAS)
        chart.paste(avatar_img, (ava_x, ava_y))
