...
async def paragon(cmd, message, args):
    ...
    try:
        ...
        try: core_kills = stats['kills_core']
        except: core_kills = None
            
        response = discord.Embed(color=0xEEEEEE)
        response.set_author(name=f'{profile_data["displayName"]} | Level {stats["account_level_up"]}', icon_url=pgn_icon)
