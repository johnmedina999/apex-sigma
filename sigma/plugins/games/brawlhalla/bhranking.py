...
async def bhranking(cmd, message, args):
    ...
    if not args: region = 'global'
    else:
        region = args[0].lower()
        if region not in regions:
            embed = discord.Embed(color=0xDB0000)
            embed.add_field(name='❗ Invalid Region', value='```\nRegions: ' + ', '.join(regions).upper() + '\n```')
            await message.channel.send(None, embed=embed)
            return
    
    if region == 'global': lb_url = url_base
    else: lb_url = url_base + region + '/'
    ...
    for x in range(0, 10):
        data = rankings[x]
        out = '{:s}: {:s} | Season: {:s} | Peak: {:s}'.format(data['Region'], data['Name'], data['Season'], data['Peak'])
        player_list += '\n' + out
