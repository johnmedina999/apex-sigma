...
async def pokemon(cmd, message, args):
    ...
    if len(poke_input) == 0: await message.channel.send(cmd.help()); return
    ...
    for ability in poke['abilities']:
        if ability['is_hidden']: hidden = 'Hidden'
        else: hidden = 'Visible'
    ...
    try:        
        ...
        await message.channel.send('Error retrieving pokemon info')
        return
