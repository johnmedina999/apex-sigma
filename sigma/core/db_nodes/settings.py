...
def add_new_server_settings_node(db, server):
    server_db = db['ServerSettings'].find({ 'ServerID': str(server.id) })
...
def set_settings_node(db, server_id, setting, setting_variable):
    collection = 'ServerSettings'
    updatetarget = { 'ServerID': str(server_id) }
...
def check_for_missing_settings_node(db, server):
    ...
    for key in default:
        try: get_settings_node(db, str(server.id), key)
        except:
            value = default[key]
            set_settings_node(db, str(server.id), key, value)
